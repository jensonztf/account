...
<div style="width:auto;float:left;"><table id="excel" width='60px' border='0' align='left' cellpadding='5' cellspacing='1' bgcolor='#B3B3B3' style="float:left" class='table table-striped table-bordered' >
                <tr>
                <th bgcolor='#EBEBEB'>
...
$ntjxz=date("Y");
echo "<option value='$ntjnian'>年份</option>";
...
//支出分类
$zhichu=array();
$sqlshouru="select * from jizhang_account_class where ufid='$_SESSION[uid]' and classtype='2'";
...
<div style="width:auto;float:left;margin-left: -1px;"><table id="excel" width='60px' border='0' align='left' cellpadding='5' cellspacing='1' bgcolor='#B3B3B3' style="float:left" class='table table-striped table-bordered' >
...
$j="$ntjxz-1-31";
$k="$ntjxz-1-1";
$sqltime=" jizhang_account.actime >".strtotime($k." 0:0:0")." and jizhang_account.actime <".strtotime($j." 23:59:59");
//收入分类
$sql="select sum(acmoney) as total,acclassid,jizhang_account_class.classname from jizhang_account left join jizhang_account_class on jizhang_account.acclassid =jizhang_account_class.classid where jizhang_account_class.classtype =1 and ".$sqltime." and jiid='$_SESSION[uid]' group by jizhang_account.acclassid";
...
$sql="select sum(acmoney) as total,acclassid,jizhang_account_class.classname from jizhang_account left join jizhang_account_class on jizhang_account.acclassid =jizhang_account_class.classid where jizhang_account_class.classtype =2 and ".$sqltime." and jiid='$_SESSION[uid]' group by jizhang_account.acclassid";
$s2=array();
$query=mysqli_query($conn,$sql);
			while($acclass=mysqli_fetch_array($query)){
$total = $acclass['total'];
